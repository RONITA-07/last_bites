...
router.post('/place', async (req, res) => {
  try {
    const { user_id, food_id, quantity, discount } = req.body;
    ...
    const totalPrice = Math.max(0, Number((Number(listing.price) * qty - disc).toFixed(2)));
    ...
    // Credit coins: 1 coin per quantity rescued
    const user = await User.findById(user_id);
    if (user) {
      const currentCoins = Number(user.coinsBalance || 0);
      const coinsEarned = qty * 1;
      await User.findByIdAndUpdate(user_id, {
        $set: { coinsBalance: currentCoins + coinsEarned }
      });
      console.log(`[Coins System] Credited ${coinsEarned} coins to User ${user_id}. New balance: ${currentCoins + coinsEarned}`);
    }
